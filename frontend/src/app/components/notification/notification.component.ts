...
export class NotificationComponent implements OnInit {
    ...
    opened(item: NotificationAddresses) {
        ...
        if (item.notification?.answer?.id) {
            item.isWriteAnswer = true;
        }
    ...
    answerBtnClick(item: NotificationAddresses) {
        ...
        const data: NotesDto = {
            ...
            text: item.notification.answer?.message,
    ...
    showWriteAnswer(item) {
        if (!item.notification.answer) {
            item.notification.answer = {}
        }
        item.isWriteAnswer = !item.isWriteAnswer
    }
}
